...
def main():
    ...
    with open(output_file, 'w') as f:
        ...
        for i, frame in enumerate(padded_frames):
            ...
            # Prepare block data for writing using NumPy
            block_data = np.zeros((len(frame_blocks), 193), dtype=np.int32)
            for j, (block_type, r_coeffs, g_coeffs, b_coeffs) in enumerate(frame_blocks):
                block_data[j, 0] = block_type
                block_data[j, 1:65] = r_coeffs
                block_data[j, 65:129] = g_coeffs
                block_data[j, 129:193] = b_coeffs

            # Write block data to file
            np.savetxt(f, block_data, fmt='%d')
